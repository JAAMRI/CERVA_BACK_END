...
class EncounterController extends Controller
{
    ...
    public function addNewEncounterProcedureAction($encounter_id, Request $request)
    {
        ...
        $em->flush();
        //Notify node with the new encounter procedure added
        $this->get("chronoheed.notify")->nodePushViaGet("event=selectedEncounterProcedureAdded&encounterId=" . $encounter->getId());
    ...
    public function updateEncounterProcedureAction($procedure_id, Request $request)
    {
        ...
        $em->flush();
        //Notify node with the new encounter procedure added
        $this->get("chronoheed.notify")->nodePushViaGet("event=selectedEncounterProcedureUpdated&procedureId=" . $procedure_id . "&encounterId=" . $encounterProcedure->getEncounter()->getId() . "&patientId=" . $encounterProcedure->getEncounter()->getPatient()->getId());
        return array("status" => "success");
        //
    }
     ...
     * @View
     */
    public function deleteEncounterProcedureAction($procedure_id, Request $request)
    {
        $em = $this->getDoctrine()->getEntityManager();
        //Get Encounter procedure
        ...
        $em->flush();
        //Notify node with the new encounter procedure added
        $this->get("chronoheed.notify")->nodePushViaGet("event=selectedEncounterProcedureDeleted&procedureId=" . $procedure_id . "&encounterId=" . $encounterProcedure->getEncounter()->getId());
        return array("status" => "success");
...
//        $encounter_data = $req_data['encounter'];
        ...
        $this->get("chronoheed.notify")->nodePushViaGet("event=encounterStatusChange&encounterId=" . $encounter->getId() . "&patientId=" . $encounter->getPatient()->getId() . "&encounterStatus=" . $encounter->getStatus() . "&encounterCheckin=" . $encounter->getCheckinTime()->format("Y-m-d") . "&senderId=" . $senderId);
    ...
    public function updateEncounterNotesAction($encounter_id, Request $request)
    {
        ...
        $em->flush();
        //Notify node with the new encounter procedure added
        $this->get("chronoheed.notify")->nodePushViaGet("event=selectedEncounterNotesUpdated&encounterId=" . $encounter->getId() . "&patientId=" . $encounter->getPatient()->getId() . "&checkinTime=" . $encounter->getCheckinTime()->format("Y-m-d"));

        return array("success" => true);
    }

    /**
     * @Get("api/patients/{patient_id}/encounters/notes")
     * @param patient_id
     * @View
     */
    public function returnPatientEncountersNotesAction($patient_id)
    {
        if(!$patient = $this->getDoctrine()->getManager()->find("AppBundle:Patient", $patient_id)){
            return new JsonResponse([
                'error' => 'bad_request', 'error_description' => 'Patient not found'
            ], 400);
        }
        else
        return $this->getDoctrine()->getRepository("AppBundle:Encounter")->returnEncountersNotesPerPatient($patient);
     ...
     * @View
     */
    public function returnPatientEncountersAction($patient_id)
    {
        if(!$patient = $this->getDoctrine()->getManager()->find("AppBundle:Patient", $patient_id)){
            return new JsonResponse([
                'error' => 'bad_request', 'error_description' => 'Patient not found'
        ...
        $encounters = $this->getDoctrine()->getRepository("AppBundle:Encounter")->returnEncountersPerPatient($patient);

        if($encounters){
            return $this->formatPatientEncountersForDataTable($encounters);
        }
        return array('data' => "");
    }

    private function formatPatientEncountersForDataTable ($results){
        $array_ = [];
        foreach ($results as $row){
            $rowArr = [];
            //Set appointment id as row id
