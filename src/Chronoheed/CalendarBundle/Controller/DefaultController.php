...
//        if(!isset($event_data['start']) || !isset($event_data['end']) || !isset($event_data['type']) || !isset($event_data['provider']) || (!isset($event_data['lastname']) && !isset($event_data['patient'])) )
        ...
        try {
            ...
            if ($form->isValid()) {
                try {
                    ...
                    $em->persist($event);
                    $em->flush();
                    //Notify other users via node push
                    $senderId = $request->query->get('senderId');//Get client id from URL
                    $this->get("chronoheed.notify")->nodePushViaGet("event=newEventAdded&start=" . $event->getStart()->format("Y-m-d") . "&senderId=" . $senderId);
                    //Add procedure abbreviation to the event title
    ...
    public function returnEventsAction(Request $request)
    {
        ...
        $results = $this->getDoctrine()->getRepository("AppBundle:Event")->returnEvents($start, $end, $provider);

        return $this->adaptForFullCalendar($results);
    }

    private function adaptForFullCalendar($results){

        $array_ = [];
        foreach ($results as $row) {
            $rowArr = [];
            $rowArr['id'] = $row['id'];
            $rowArr['start'] = $row['start'];
            $rowArr['end'] = $row['end'];
            $rowArr['color'] = $row['color'];
            $rowArr['phone'] = $row['phone'];
            $rowArr['lastname'] = $row['lastname'];
            $rowArr['type'] = $row['type'];
            $rowArr['procedures'] = $row['procedures'];
            $rowArr['title'] = $row['title'] . "AL HAMDOLILLAH YA RABBI";
            //Get all default procedures code in an array
            $defaultProcedures = $this->getDoctrine()->getRepository("AppBundle:DefaultProcedure")->findAll();
            $defaultProceduresCodesPerId = [];
            foreach($defaultProcedures as $defaultProcedure){
                $defaultProceduresCodesPerId[$defaultProcedure->getId()] = $defaultProcedure->getCode();
            }//End foreach
            if($rowArr['procedures'] != ""){//Make sure there are procedures for this event
                $procedures_codes_for_calendar_title = $this->returnProceduresCodesForCalendarEventTitle($rowArr['procedures'], $defaultProceduresCodesPerId);
                //Add procedures codes to title
                if($procedures_codes_for_calendar_title != ""){
                    $rowArr['title'] = $rowArr['title']  . "AL HAMDOLILLAH" . $procedures_codes_for_calendar_title;
                }
            }
    ...
    //This function
    private function returnProceduresCodesForCalendarEventTitle($procedureIdsList, $defaultProceduresCodesPerId){
        //Procedures codes to be returned as a string
        $procedures_codes_for_calendar_title = "";//String
        $procedureIdsArr = explode("|", $procedureIdsList);
        if(count($procedureIdsArr) > 0){
            foreach($procedureIdsArr as $procedureId) {
                $procedures_codes_for_calendar = $procedures_codes_for_calendar_title . " - " . $defaultProceduresCodesPerId[$procedureId];
            }
        }
        return $procedures_codes_for_calendar_title;
    ...
    public function updateEventAction(Request $request, $eventId)
    {
        try{
            ...
            if($event){
                ...
                if ($form->isValid()) {
                    ...
                    try {
                        ...
                        //Notify other users via node push
                        $senderId = $request->query->get('senderId');//Get client id from URL
                        $this->get("chronoheed.notify")->nodePushViaGet("event=eventModified&start=" . $event->getStart()->format("Y-m-d") . "&senderId=" . $senderId);
                        return $event;
                    }catch (PDOException $e){
                        return new JsonResponse([
                            'error' => 'bad_request', 'error_description' => $e->getMessage()
            ...
            return new JsonResponse([
                'error' => 'Unknown_error', 'error_description' => $e->getMessage()
            ], 500);
        }
    }
     ...
     * @View
     */
    public function deleteEventAction($eventId, Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $event = $em->find("AppBundle:Event", $eventId);
        if($event->getStatus() == "CHECKED_IN"){
        ...
        if($event){
            ...
            //Notify other users via node push
            $senderId = $request->query->get('senderId');//Get client id from URL
            $this->get("chronoheed.notify")->nodePushViaGet("event=eventDeleted&start=" . $start->format("Y-m-d") . "&senderId=" . $senderId);
            return new JsonResponse([
                'error' => 'bad_request', 'error_description' => 'resource not found'
            ], 204);
        }
...
//            $encounter = $em->find("AppBundle:Encounter", 40);
            ...
            if (! $patient = $event->getPatient()) {//this is null => this is a new patient
                ...
                if ($patient = $this->createPatient($patient_data)) {//Patient created
                    ...
                    $event->setPatient($patient);
                    //Updating the event with the created patient at this level is mandatory for the encounter creation to go through
                    $em->persist($event);
                    $em->flush();
                } else {
            ...
            $event->setPhone($patient->getCellphone());
            $em->persist($event);
            $em->flush();
